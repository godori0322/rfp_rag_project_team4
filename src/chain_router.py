...
class ChainRouter:
    ...
    def create_router_chain(self):
        ...
        route_prompt = ChatPromptTemplate.from_template(
            ...
            # 카테고리 설명:
            `metadata_search`: 특정 RFP 문서의 메타데이터(사업명, 사업 금액, 발주 기관, 공고 번호, 날짜 등) 자체를 찾아달라는 요청.
            `summarization`: 문서의 전반적인 내용, 특정 섹션, 또는 주요 요구사항 등을 요약해달라는 요청.
            `comparison`: 두 개 이상의 문서 또는 하나의 문서 내 두 개 이상의 항목을 비교/대조해달라는 요청.
            `recommendation`: 특정 사업과 유사한 다른 사업을 추천해달라는 요청.
            `default_qa`: RFP 문서의 특정 내용에 대한 상세하고 구체적인 정보를 묻는 질문. (위 4가지에 해당하지 않는 모든 RFP 관련 질문)
